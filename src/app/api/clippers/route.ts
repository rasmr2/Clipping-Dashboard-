...
import { CreateClipperSchema, validateBody } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  ...
  const validation = await validateBody(request, CreateClipperSchema);
  ...
  try {
    ...
    logRequest(request, 500, startTime, String(error));
    return NextResponse.json({ error: "Failed to create clipper" }, { status: 500 });
  }
}
